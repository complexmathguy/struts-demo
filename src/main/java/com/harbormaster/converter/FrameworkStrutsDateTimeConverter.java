            ...
            if (indate.length() > 0) {
                try {
                    if (toClass.getName().equals("java.sql.Timestamp")) {
                        return new java.sql.Timestamp(new java.text.SimpleDateFormat(
                                FrameworkStrutsDateTimeConverter.timeFormat).parse(
                                indate).getTime());
                    } else if (toClass.getName().equals("java.util.Date")) {
                        java.util.Calendar cal = java.util.Calendar.getInstance();
                        cal.setTime(new java.text.SimpleDateFormat(
                                FrameworkStrutsDateTimeConverter.dateTimeFormat).parse(
                                indate));

                        return new java.sql.Date(cal.getTime().getTime());
                    }
                } catch (Throwable e) {
                    LOGGER.severe("- error converting value [" + values[0] +
                        "] to Date " + e);
                }
            }
        }
