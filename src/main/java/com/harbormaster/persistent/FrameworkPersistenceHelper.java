...
public class FrameworkPersistenceHelper {
    ...
    protected static void createSessionFactory() {
        try {
            ...
            StandardServiceRegistryBuilder standardRegistryBuilder = new StandardServiceRegistryBuilder().configure(
                    "hibernate.cfg.xml");

            if (System.getenv("DATABASE_URL") != null) {
                LOGGER.info("Using DATABASE_URL " +
                    System.getenv("DATABASE_URL"));
                standardRegistryBuilder = standardRegistryBuilder.applySetting("hibernate.connection.url",
                        System.getenv("DATABASE_URL"));
            }

            if (System.getenv("DATABASE_USERNAME") != null) {
                LOGGER.info("Using DATABASE_USERNAME " +
                    System.getenv("DATABASE_USERNAME"));
                standardRegistryBuilder = standardRegistryBuilder.applySetting("hibernate.connection.username",
                        System.getenv("DATABASE_USERNAME"));
            }

            if (System.getenv("DATABASE_PASSWORD") != null) {
                LOGGER.info("Using DATABASE_PASSWORD " +
                    System.getenv("DATABASE_PASSWORD"));
                standardRegistryBuilder = standardRegistryBuilder.applySetting("hibernate.connection.password",
                        System.getenv("DATABASE_PASSWORD"));
            }

            if (System.getenv("DATABASE_DIALECT") != null) {
                LOGGER.info("Using DATABASE_DIALECT " +
                    System.getenv("DATABASE_DIALECT"));
                standardRegistryBuilder = standardRegistryBuilder.applySetting("hibernate.dialect",
                        System.getenv("DATABASE_DIALECT"));
            }

            if (System.getenv("DATABASE_DRIVER") != null) {
                LOGGER.info("Using DATABASE_DRIVER " +
                    System.getenv("DATABASE_DRIVER"));
                standardRegistryBuilder = standardRegistryBuilder.applySetting("hibernate.connection.driver_class",
                        System.getenv("DATABASE_DRIVER"));
            }
            ...
            LOGGER.log(Level.SEVERE,
                "Warning!!! - Hibernate Session Failed to Initialize", exc);
        }
    }
}
